...
private val x = List(11) { Day12.OK }
...
fun main() {
    ...
    data class SearchState(val currentPosition: Int, val currentGroupIdx: Int, val currentGroupLength: Int) {
        override fun toString() =
            "SearchState(pos=$currentPosition, groupIdx=$currentGroupIdx, curGrpLen=$currentGroupLength)"
    }


    fun countPermutations(it: Line): Long {
        val (states, groups) = it
        val countValidPermutations: DeepRecursiveFunction<SearchState, Long> =
            MemoizedDeepRecursiveFunction { (currentPosition, currentGroupIdx, currentGroupLength) ->
                ...
                if (currentPosition == states.size) // end of the source
                    ...
                    when (states[currentPosition]) {
                        Day12.OK -> countDotPermutations()
                        Day12.DAMAGED -> countHashPermutations()
                        Day12.UNKNOWN -> countDotPermutations() + countHashPermutations()
                    }

            }
        val result =
            countValidPermutations(SearchState(currentPosition = 0, currentGroupIdx = 0, currentGroupLength = 0))
        return result
    }
    ...
    fun part2(test: List<Line>): Long = test
        .sumOf { (a, b) ->
            val line = Line(
                (1..5).map { a }.reduce { acc, day12s -> acc + Day12.UNKNOWN + day12s },
    ...
    val test = parser.parseOrThrow(readInputTxt("src/main/kotlin/year_2023/day12.txt"))
//    check(part1(test) == 21L)
    val input = parser.parseOrThrow(readInputTxt("src/main/kotlin/year_2023/day12.txt"))
//    part1(input).println()
